...
def plot_Voltagevstime(times,
    ...
    voltage_legend : str, optionnel
        Texte de la légende tension.
    ax : Axes, optionnel
        Axe sur lequel tracer (si on ne veut pas créer une nouvelle figure).
    """
    # Appliquer le slicing si demandé
    if time_slice is not None:
        start, end = time_slice
    ...
    # Créer une figure/axe seulement si nécessaire
    if ax is None:
        fig, ax = plt.subplots(figsize=figsize)
    else:
        fig = ax.figure
    ...
    # Tension
    ax.plot(times, voltages, color=voltage_color, marker = marker, label=voltage_legend, linewidth=1.5)
...
def plot_drt(tau, gamma, title="DRT", ax = None, color = sns.color_palette("deep")[0], label = None):
    sns.set_palette("Set2")
    if ax is None:
        fig, ax = plt.subplots(figsize=(7,4))
    ax.semilogx(tau, gamma, '-',label = label,color=color, linewidth=1.2)
